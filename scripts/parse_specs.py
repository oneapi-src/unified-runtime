...
def _generate_structure_type(specs, file):
    extended_structs = [obj for s in specs for obj in s['objects'] if re.match(r"struct|union", obj['type']) and 'base' in obj]
    in_yml = list(util.yamlRead(file))
    structure_type = [item for item in in_yml if item.get('name') == "$x_structure_type_t"][0]

    requires_update = False
    for struct in extended_structs:
        stype = [mem for mem in struct['members'] if mem['name'] == 'stype'][0]
        etor = stype['init'][len("$X_STRUCTURE_TYPE_"):]
        if etor not in [e['name'] for e in structure_type['etors']]:
            ## skip experimental features
            if etor.startswith("EXP_"):
                continue
            requires_update = True
            structure_type['etors'].append({"name": etor, "desc": struct['name']})
    
    if requires_update:
        util.yamlWrite(file, in_yml,default_flow_style=False,
                sort_keys=False,
                explicit_start=True)
...
def parse(section, version, tags, meta, ref):
    ...
    def sort_files(file):
        ...
        if file.endswith("registry.yml"):
            return 2
        # make sure structure is first, so that values are available to be used
        if file.endswith("structure.yml"):
            return 0
        return 1
    ...
    structure_file = [f for f in files if f.endswith("structure.yml")][0]
    if _generate_structure_type(specs, structure_file):
        return parse(section, version, tags, {}, {})
