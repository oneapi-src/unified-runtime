...
TEST_P(urProgramLinkWithParam, Success) {
  ...
  if (backend == UR_PLATFORM_BACKEND_OPENCL && option == "-O0") {
    option =
        "-cl-opt-disable"; // -O0 is equivalent to -cl-opt-disable in OpenCL
  }

  const char *platformOption = nullptr;
  ASSERT_SUCCESS(
      urPlatformGetBackendOption(platform, option.c_str(), &platformOption));
