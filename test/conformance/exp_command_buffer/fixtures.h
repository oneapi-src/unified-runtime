...
struct urUpdatableCommandBufferExpExecutionTest
  ...
  void SetUp() override {
    ...
    // Currently there are synchronization issue with immediate submission when
    // used for command buffers. So, create queue with batched submission for
    // this test suite if the backend is Level Zero.
