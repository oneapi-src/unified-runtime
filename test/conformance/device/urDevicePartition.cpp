...
TEST_F(urDevicePartitionTest, PartitionEquallySuccess) {
    for (auto device : devices) {
        ...
        for (uint32_t i = 1; i < n_compute_units; ++i) {
            ...
            uint32_t n_devices;
            ASSERT_SUCCESS(
                urDevicePartition(device, &part_desc, 0, nullptr, &n_devices));
            ASSERT_NE(n_devices, 0);

            std::vector<ur_device_handle_t> sub_devices(n_devices);
            ASSERT_SUCCESS(urDevicePartition(
                device, &part_desc, static_cast<uint32_t>(sub_devices.size()),
...
TEST_F(urDevicePartitionTest, PartitionByCounts) {
    ...
    for (auto device : devices) {
        ...
        ur_device_partition_by_counts_desc_t part_by_counts_desc{
            UR_STRUCTURE_TYPE_DEVICE_PARTITION_BY_COUNTS_DESC, nullptr, 0};

        ur_device_partition_desc_t part_desc{
            UR_STRUCTURE_TYPE_DEVICE_PARTITION_DESC, &part_by_counts_desc};
        ...
        for (const auto Combination : combinations) {
            ...
            case Combination::HALF: {
                ...
                counts.push_back(n_cu_in_device / 2);
                counts.push_back(n_cu_in_device / 2);
                break;
            }
            case Combination::ALL_MINUS_ONE: {
                n_cu_across_sub_devices = n_cu_in_device - 1;
                counts.push_back(n_cu_in_device - 1);
                break;
            }
            case Combination::ALL: {
                n_cu_across_sub_devices = n_cu_in_device;
                counts.push_back(n_cu_in_device);
                break;
            }
            }

            part_by_counts_desc.counts = counts.data();
            part_by_counts_desc.size = counts.size();

            // Get the number of devices that will be created
            uint32_t n_devices;
            ASSERT_SUCCESS(
                urDevicePartition(device, &part_desc, 0, nullptr, &n_devices));
            ASSERT_EQ(n_devices, counts.size());
...
TEST_P(urDevicePartitionAffinityDomainTest, PartitionByAffinityDomain) {
    ...
    for (auto device : devices) {
        ...
        ur_device_partition_by_affinity_domain_desc_t part_aff_desc{
            UR_STRUCTURE_TYPE_DEVICE_PARTITION_BY_AFFINITY_DOMAIN_DESC, nullptr,
            flag};
        ur_device_partition_desc_t part_desc{
            UR_STRUCTURE_TYPE_DEVICE_PARTITION_DESC, &part_aff_desc};
...
TEST_F(urDevicePartitionTest, InvalidNullHandleDevice) {
    ur_device_partition_equally_desc_t part_eq_desc{
        UR_STRUCTURE_TYPE_DEVICE_PARTITION_EQUALLY_DESC, nullptr, 1};
    ur_device_partition_desc_t part_desc{
        UR_STRUCTURE_TYPE_DEVICE_PARTITION_DESC, &part_eq_desc};
    ...
    ASSERT_EQ_RESULT(
        ...
        urDevicePartition(nullptr, &part_desc, 1, &sub_device, nullptr));
}
