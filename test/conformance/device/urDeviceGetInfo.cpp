...
TEST_P(urDeviceGetInfoTest, SuccessSubGroupSizesIntel) {
  ...
  uint32_t returned_groups = 999;
  ASSERT_SUCCESS(urDeviceGetInfo(device, property_name, property_size,
                                 &returned_groups, nullptr));

  ASSERT_NE(returned_groups, 999);
}
...
TEST_P(urDeviceGetInfoTest, SuccessUUID) {
  ...
  std::vector<uint8_t> property_value(property_size, 'x');
  ASSERT_SUCCESS(urDeviceGetInfo(device, property_name, property_size,
                                 property_value.data(), nullptr));
  EXPECT_NE(property_value[0], 'x');
}
