...
using urEnqueueUSMAdviseTest = uur::urUSMDeviceAllocTest<ur_usm_advice_flag_t>;
UUR_TEST_SUITE_P(urEnqueueUSMAdviseTest,
                 ::testing::Values(UR_USM_ADVICE_FLAG_DEFAULT),
                 uur::deviceTestPrinter<ur_usm_advice_flag_t>);

TEST_P(urEnqueueUSMAdviseTest, Success) {
...
using urEnqueueUSMAdviseNegativeTest = uur::urUSMDeviceAllocTest<>;
UUR_INSTANTIATE_DEVICE_TEST_SUITE_P(urEnqueueUSMAdviseNegativeTest);

TEST_P(urEnqueueUSMAdviseNegativeTest, MultipleParamsSuccess) {
...
TEST_P(urEnqueueUSMAdviseNegativeTest, InvalidNullHandleQueue) {
    ASSERT_EQ_RESULT(UR_RESULT_ERROR_INVALID_NULL_HANDLE,
                     urEnqueueUSMAdvise(nullptr, ptr, allocation_size,
                                        UR_USM_ADVICE_FLAG_DEFAULT, nullptr));
}

TEST_P(urEnqueueUSMAdviseNegativeTest, InvalidNullPointerMem) {
    ASSERT_EQ_RESULT(UR_RESULT_ERROR_INVALID_NULL_POINTER,
                     urEnqueueUSMAdvise(queue, nullptr, allocation_size,
                                        UR_USM_ADVICE_FLAG_DEFAULT, nullptr));
}

TEST_P(urEnqueueUSMAdviseNegativeTest, InvalidEnumeration) {
...
TEST_P(urEnqueueUSMAdviseNegativeTest, InvalidSizeZero) {
    ASSERT_EQ_RESULT(
        UR_RESULT_ERROR_INVALID_SIZE,
        urEnqueueUSMAdvise(queue, ptr, 0, UR_USM_ADVICE_FLAG_DEFAULT, nullptr));
}

TEST_P(urEnqueueUSMAdviseNegativeTest, InvalidSizeTooLarge) {
    ASSERT_EQ_RESULT(UR_RESULT_ERROR_INVALID_SIZE,
                     urEnqueueUSMAdvise(queue, ptr, allocation_size * 2,
                                        UR_USM_ADVICE_FLAG_DEFAULT, nullptr));
}

TEST_P(urEnqueueUSMAdviseNegativeTest,
       NonCoherentDeviceMemorySuccessOrWarning) {
    ur_result_t result =
        urEnqueueUSMAdvise(queue, ptr, allocation_size,
