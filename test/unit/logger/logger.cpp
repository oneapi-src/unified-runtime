...
TEST_F(DefaultLoggerWithFileSink, SetLevelError) {
    logger->setLevel(UR_LOGGER_LEVEL_ERR);
    logger->error("Test message: {}", "success");
...
TEST_P(CommonLoggerWithMultipleThreads, StdoutMultithreaded) {
    std::vector<std::thread> threads;
    auto local_logger =
        logger::Logger(UR_LOGGER_LEVEL_WARN,
...
TEST_F(LoggerWithCallbackSink, CallbackSinkTest) {
    // Pass a callback function to the logger which will be used as an additional destination sink
    // and any logged messages will be sent to the callback function
