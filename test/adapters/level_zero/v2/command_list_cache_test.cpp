...
TEST_P(CommandListCacheTest, CanStoreAndRetriveImmediateAndRegularCmdLists) {
  ...
  for (int i = 0; i < numListsPerType; ++i) {
    ...
    std::tie(it, _) = immCmdLists.emplace(
        cache
            .getImmediateCommandList(device->ZeDevice, IsInOrder, Ordinal, Mode,
                                     Priority)
            .release());
    ASSERT_TRUE(*it != nullptr);
  }
  ...
  for (int i = 0; i < numListsPerType; ++i) {
    ...
    auto immCmdList = cache.getImmediateCommandList(device->ZeDevice, IsInOrder,
                                                    Ordinal, Mode, Priority);
...
TEST_P(CommandListCacheTest, ImmediateCommandListsHaveProperAttributes) {
  ...
  for (uint32_t Ordinal = 0; Ordinal < numQueueGroups; Ordinal++) {
    ...
    for (uint32_t Index = 0; Index < QueueGroupProperties[Ordinal].numQueues;
      ...
      cache.addImmediateCommandList(std::move(CommandList), device->ZeDevice,
                                    IsInOrder, Ordinal, Mode, Priority, Index);
    }
