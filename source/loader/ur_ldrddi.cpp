...
__urdlllocal ur_result_t UR_APICALL urCommandBufferRetainCommandExp(
  ...
  auto *pfnRetainCommandExp = dditable->CommandBufferExp.pfnRetainCommandExp;
...
__urdlllocal ur_result_t UR_APICALL urCommandBufferReleaseCommandExp(
  ...
  auto *pfnReleaseCommandExp = dditable->CommandBufferExp.pfnReleaseCommandExp;
...
__urdlllocal ur_result_t UR_APICALL urCommandBufferCommandGetInfoExp(
  ...
  auto *pfnCommandGetInfoExp = dditable->CommandBufferExp.pfnCommandGetInfoExp;
...
__urdlllocal ur_result_t UR_APICALL urTensorMapEncodeIm2ColExp(
  ...
  auto *pfnEncodeIm2ColExp = dditable->TensorMapExp.pfnEncodeIm2ColExp;
...
__urdlllocal ur_result_t UR_APICALL urTensorMapEncodeTiledExp(
  ...
  auto *pfnEncodeTiledExp = dditable->TensorMapExp.pfnEncodeTiledExp;
...
UR_DLLEXPORT ur_result_t UR_APICALL urGetTensorMapExpProcAddrTable(
  ...
  for (auto &platform : ur_loader::getContext()->platforms) {
    ...
    if (!getTable)
      continue;
    platform.initStatus = getTable(version, &platform.dditable.TensorMapExp);
  }
  ...
  if (UR_RESULT_SUCCESS == result) {
    if (ur_loader::getContext()->platforms.size() != 1 ||
      ...
      // return pointers directly to platform's DDIs
      *pDdiTable =
          ur_loader::getContext()->platforms.front().dditable.TensorMapExp;
    }
  }
