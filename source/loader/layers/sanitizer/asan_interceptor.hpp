...
struct DeviceInfo {
  ...
  explicit DeviceInfo(ur_device_handle_t Device) : Handle(Device) {
    [[maybe_unused]] auto Result = context.urDdiTable.Device.pfnRetain(Device);
    assert(Result == UR_RESULT_SUCCESS);
  }

  ~DeviceInfo() {
    [[maybe_unused]] auto Result = context.urDdiTable.Device.pfnRelease(Handle);
    assert(Result == UR_RESULT_SUCCESS);
  }
...
struct QueueInfo {
  ...
  explicit QueueInfo(ur_queue_handle_t Queue)
    ...
    [[maybe_unused]] auto Result = context.urDdiTable.Queue.pfnRetain(Queue);
    assert(Result == UR_RESULT_SUCCESS);
  }
...
struct KernelInfo {
  ...
  explicit KernelInfo(ur_kernel_handle_t Kernel) : Handle(Kernel) {
    [[maybe_unused]] auto Result = context.urDdiTable.Kernel.pfnRetain(Kernel);
    assert(Result == UR_RESULT_SUCCESS);
  }

  ~KernelInfo() {
    [[maybe_unused]] auto Result = context.urDdiTable.Kernel.pfnRelease(Handle);
    assert(Result == UR_RESULT_SUCCESS);
  }
};
