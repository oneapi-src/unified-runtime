...
// void PrintAllocateInfo(uptr Addr, const MsanAllocInfo *AI) {
...
//                    const std::shared_ptr<MsanAllocInfo> &AI) {
...
// void ReportMemoryLeak(const std::shared_ptr<MsanAllocInfo> &AI) {
//     getContext()->logger.always(
...
//     if (getMsanInterceptor()->getOptions().MaxQuarantineSizeMB > 0) {
//         auto MsanAllocInfoItOp =
//             getMsanInterceptor()->findMsanAllocInfoByAddress(Report.Address);

//         if (!MsanAllocInfoItOp) {
//             getContext()->logger.always(
//                 "Failed to find which chunck {} is allocated",
//                 (void *)Report.Address);
//         } else {
//             auto &MsanAllocInfo = (*MsanAllocInfoItOp)->second;
//             if (MsanAllocInfo->Context != Context) {
//                 getContext()->logger.always(
//                     "Failed to find which chunck {} is allocated",
//                     (void *)Report.Address);
//             }
//             assert(MsanAllocInfo->IsReleased &&
//                    "It must be released since it's use-after-free");

//             PrintAllocateInfo(Report.Address, MsanAllocInfo.get());
//         }
//     } else {
//         getContext()->logger.always(
