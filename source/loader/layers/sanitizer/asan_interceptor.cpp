...
uptr MemToShadow_PVC(uptr USM_SHADOW_BASE, uptr UPtr) {
  if (UPtr & 0xFF00000000000000ULL) { // Device USM
    ...
    return USM_SHADOW_BASE + ((UPtr & 0x7FFFFFFFFFFFULL) >> ASAN_SHADOW_SCALE);
  }
}
...
ur_result_t enqueueMemSetShadow(ur_context_handle_t Context,
  ...
  if (DeviceInfo->Type == DeviceType::CPU) {
    ...
    uptr ShadowEnd = MemToShadow_CPU(DeviceInfo->ShadowOffset, Ptr + Size - 1);
    ...
    uptr ShadowEnd = MemToShadow_PVC(DeviceInfo->ShadowOffset, Ptr + Size - 1);
    assert(ShadowBegin <= ShadowEnd);
    {
      ...
      for (auto MappedPtr = RoundDownTo(ShadowBegin, PageSize);
        ...
        if (URes == UR_RESULT_SUCCESS) {
          ...
          auto URes = urEnqueueUSMSet(Queue, (void *)MappedPtr, 0, PageSize);
          if (URes != UR_RESULT_SUCCESS) {
...
ur_result_t SanitizerInterceptor::releaseMemory(ur_context_handle_t Context,
  ...
  if (ReleaseList.size()) {
    ...
    for (auto &It : ReleaseList) {
      ...
      UR_CALL(context.urDdiTable.USM.pfnFree(Context,
                                             (void *)(It->second->AllocBegin)));
    }
  }
...
SanitizerInterceptor::registerDeviceGlobals(ur_context_handle_t Context,
  ...
  for (auto Device : Devices) {
    ...
    auto Result = context.urDdiTable.Enqueue.pfnDeviceGlobalVariableRead(
        ...
        sizeof(NumOfDeviceGlobal), 0, &NumOfDeviceGlobal, 0, nullptr, nullptr);
    if (Result != UR_RESULT_SUCCESS) {
...
ur_result_t SanitizerInterceptor::prepareLaunch(
  ...
  do {
    ...
    if (ArgNums) {
      context.logger.debug("launch_info {} (numLocalArgs={}, localArgs={})",
                           (void *)LaunchInfo.Data,
                           LaunchInfo.Data->NumLocalArgs,
    ...
    auto EnqueueWriteGlobal = [Queue, Program](const char *Name,
      ...
      auto Result = context.urDdiTable.Enqueue.pfnDeviceGlobalVariableWrite(
          Queue, Program, Name, false, Size, 0, Value, 0, nullptr, nullptr);
      if (Result != UR_RESULT_SUCCESS) {
        context.logger.warning("Failed to write device global \"{}\": {}", Name,
                               Result);
        return false;
      }
      return true;
    ...
    // Write debug
    // We use "uint64_t" here because EnqueueWriteGlobal will fail when it's
    // "uint32_t"
    ...
    auto EnqueueAllocateShadowMemory = [Context, &DeviceInfo,
      ...
      auto URes = context.urDdiTable.USM.pfnDeviceAlloc(
          Context, DeviceInfo->Handle, nullptr, nullptr, Size, &Allocated);
      if (URes != UR_RESULT_SUCCESS) {
        return URes;
      }
    ...
    context.logger.info("KernelInfo {} (LocalMemory={}, PrivateMemory={})",
                        (void *)Kernel, LocalMemoryUsage, PrivateMemoryUsage);
    ...
    if (Options().DetectLocals) {
      ...
      if (DeviceInfo->Type == DeviceType::GPU_PVC) {
        ...
        context.logger.debug("LocalMemory(WorkGroup={}, LocalMemorySize={}, "
                             "LocalShadowMemorySize={})",
                             NumWG, LocalMemorySize, LocalShadowMemorySize);

        if (EnqueueAllocateShadowMemory(LocalShadowMemorySize,
          ...
          context.logger.warning(
              ...
              "large",
              NumWG);
          context.logger.warning("Skip checking local memory of kernel <{}>",
                                 GetKernelName(Kernel));
        } else {
          LaunchInfo.Data->LocalShadowOffsetEnd =
              LaunchInfo.Data->LocalShadowOffset + LocalShadowMemorySize - 1;
    ...
    if (Options().DetectPrivates) {
      if (DeviceInfo->Type == DeviceType::CPU) {
        ...
        if (EnqueueAllocateShadowMemory(PrivateShadowMemorySize,
          ...
          context.logger.warning(
              ...
              "large",
              NumWG);
          context.logger.warning("Skip checking private memory of kernel <{}>",
                                 GetKernelName(Kernel));
        } else {
          LaunchInfo.Data->PrivateShadowOffsetEnd =
              LaunchInfo.Data->PrivateShadowOffset + PrivateShadowMemorySize -
              1;
...
USMLaunchInfo::~USMLaunchInfo() {
  ...
  if (Data) {
    ...
    if (Data->LocalArgs) {
      Result = context.urDdiTable.USM.pfnFree(Context, (void *)Data->LocalArgs);
      assert(Result == UR_RESULT_SUCCESS);
    }
