...
SanitizerInterceptor::registerSpirKernels(ur_program_handle_t Program) {
    ...
    //     Queue, Program, kSPIR_AsanSpirKernelMetadata, true,
    //     sizeof(SpirKernelInfo), 0, &SKI, 0, nullptr, nullptr);
    // getContext()->logger.error("get the pointer of <{}>: {}",
    ...
    //     std::vector<char> KernelName(SKI.Size);
    //     Result = getContext()->urDdiTable.Enqueue.pfnUSMMemcpy(
    //         Queue, true, KernelName.data(), SKI.KernelName,
    //         sizeof(char) * SKI.Size, 0, nullptr, nullptr);
    //     std::cout << "SpirKernel "
    //               << std::string(KernelName.begin(), KernelName.end()) << ": "
    //               << SKI.IsInstrumented << std::endl;
    //     PI.KernelMetadataMap[std::string(
    //         KernelName.begin(), KernelName.end())] = SKI.IsInstrumented;
    // }
