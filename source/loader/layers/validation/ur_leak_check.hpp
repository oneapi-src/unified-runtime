...
private:
  ...
  void updateRefCount(T handle, enum RefCountUpdateType type,
    ...
    case REFCOUNT_INCREASE:
      if (it == counts.end()) {
        context.logger.error("Attempting to retain nonexistent handle {}", ptr);
        return;
      } else {
        it->second.refCount++;
      }
    ...
    case REFCOUNT_DECREASE:
      ...
      if (it->second.refCount < 0) {
        context.logger.error("Attempting to release nonexistent handle {}",
                             ptr);
...
public:
  ...
  void logInvalidReferences() {
    for (auto &[ptr, refRuntimeInfo] : counts) {
      ...
      context.logger.error("Handle {} was recorded for first time here:", ptr);
      for (size_t i = 0; i < refRuntimeInfo.backtrace.size(); i++) {
        context.logger.error("#{} {}", i, refRuntimeInfo.backtrace[i].c_str());
      }
    }
  }
