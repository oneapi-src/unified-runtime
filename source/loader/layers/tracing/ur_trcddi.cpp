...
__urdlllocal ur_result_t UR_APICALL urMemGetNativeHandleExp(
    ...
    ur_mem_get_native_handle_exp_params_t params = {&hMem, &hDevice,
                                                    &phNativeMem};
    uint64_t instance =
        context.notify_begin(UR_FUNCTION_MEM_GET_NATIVE_HANDLE_EXP,
                             "urMemGetNativeHandleExp", &params);

    ur_result_t result = pfnGetNativeHandleExp(hMem, hDevice, phNativeMem);

    context.notify_end(UR_FUNCTION_MEM_GET_NATIVE_HANDLE_EXP,
                       "urMemGetNativeHandleExp", &params, &result, instance);
...
///     - ::UR_RESULT_ERROR_INVALID_NULL_POINTER
///     - ::UR_RESULT_ERROR_UNSUPPORTED_VERSION
__urdlllocal ur_result_t UR_APICALL urGetMemExpProcAddrTable(
    ur_api_version_t version, ///< [in] API version requested
    ur_mem_exp_dditable_t
        *pDdiTable ///< [in,out] pointer to table of DDI function pointers
) {
    auto &dditable = ur_tracing_layer::context.urDdiTable.MemExp;
    ...
    pDdiTable->pfnGetNativeHandleExp =
        ur_tracing_layer::urMemGetNativeHandleExp;
...
ur_result_t context_t::init(ur_dditable_t *dditable,
    ...
    if (UR_RESULT_SUCCESS == result) {
        result = ur_tracing_layer::urGetMemExpProcAddrTable(
