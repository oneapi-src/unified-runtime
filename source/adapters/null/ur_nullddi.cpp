...
__urdlllocal ur_result_t UR_APICALL urAdapterGet(
  ...
  // if the driver has created a custom function, then call it instead of using
  // the generic path
  ...
  if (nullptr != pfnAdapterGet) {
    ...
    for (size_t i = 0; (nullptr != phAdapters) && (i < NumEntries); ++i)
      phAdapters[i] = reinterpret_cast<ur_adapter_handle_t>(d_context.get());
  }
...
__urdlllocal ur_result_t UR_APICALL urPlatformGetBackendOption(
  ...
  // if the driver has created a custom function, then call it instead of using
  // the generic path
  auto pfnGetBackendOption = d_context.urDdiTable.Platform.pfnGetBackendOption;
    ...
    if (pPropValue != nullptr) {
      ...
      case UR_DEVICE_INFO_PLATFORM: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_platform_handle_t>(d_context.get());
        }
      } break;
      case UR_DEVICE_INFO_PARENT_DEVICE: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_device_handle_t>(d_context.get());
        }
      } break;
  ...
  if (nullptr != pfnGetInfo) {
    ...
    if (pPropValue != nullptr) {
      ...
      case UR_MEM_INFO_CONTEXT: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_context_handle_t>(d_context.get());
        }
      } break;
      default: {
      } break;
  ...
  if (nullptr != pfnGetMemAllocInfo) {
    ...
    if (pPropValue != nullptr) {
      ...
      case UR_USM_ALLOC_INFO_POOL: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_usm_pool_handle_t>(d_context.get());
        }
      } break;
      default: {
      } break;
  ...
  if (nullptr != pfnGetInfo) {
    ...
    if (pPropValue != nullptr) {
      ...
      case UR_KERNEL_INFO_PROGRAM: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_program_handle_t>(d_context.get());
        }
      } break;
      default: {
      } break;
  ...
  if (nullptr != pfnGetInfo) {
    ...
    if (pPropValue != nullptr) {
      ...
      case UR_QUEUE_INFO_DEVICE_DEFAULT: {
        ...
        for (size_t i = 0; i < nelements; ++i) {
          handles[i] = reinterpret_cast<ur_queue_handle_t>(d_context.get());
        }
      } break;
      default: {
      } break;
...
urBindlessImagesUnsampledImageHandleDestroyExp(
  ...
  auto pfnUnsampledImageHandleDestroyExp =
      d_context.urDdiTable.BindlessImagesExp.pfnUnsampledImageHandleDestroyExp;
...
__urdlllocal ur_result_t UR_APICALL urBindlessImagesImageAllocateExp(
  ...
  if (nullptr != pfnImageAllocateExp) {
    ...
    // generic implementation
    *phImageMem = reinterpret_cast<ur_exp_image_mem_handle_t>(d_context.get());
  }
...
__urdlllocal ur_result_t UR_APICALL urBindlessImagesImageFreeExp(
  ...
  // if the driver has created a custom function, then call it instead of using
  // the generic path
  auto pfnImageFreeExp = d_context.urDdiTable.BindlessImagesExp.pfnImageFreeExp;
...
__urdlllocal ur_result_t UR_APICALL urBindlessImagesImageCopyExp(
  ...
  // if the driver has created a custom function, then call it instead of using
  // the generic path
  auto pfnImageCopyExp = d_context.urDdiTable.BindlessImagesExp.pfnImageCopyExp;
...
__urdlllocal ur_result_t UR_APICALL urBindlessImagesImportExternalSemaphoreExp(
  ...
  if (nullptr != pfnImportExternalSemaphoreExp) {
    ...
    // generic implementation
    *phInteropSemaphore =
        reinterpret_cast<ur_exp_interop_semaphore_handle_t>(d_context.get());
  }
...
__urdlllocal ur_result_t UR_APICALL urEnqueueNativeCommandExp(
  ...
  if (nullptr != pfnNativeCommandExp) {
    result = pfnNativeCommandExp(hQueue, pfnNativeEnqueue, data,
                                 numMemsInMemList, phMemList, pProperties,
                                 numEventsInWaitList, phEventWaitList, phEvent);
  } else {
    // generic implementation
    *phEvent = reinterpret_cast<ur_event_handle_t>(d_context.get());
