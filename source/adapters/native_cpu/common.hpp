...
using object_base = ur_handle_base_t_<native_cpu_ddi_getter>;
} // namespace native_cpu

// Base class to store common data
struct _ur_object : native_cpu::object_base {
  ur_shared_mutex Mutex;
};

// Todo: replace this with a common helper once it is available
struct RefCounted : native_cpu::object_base {
  std::atomic_uint32_t _refCount;
  uint32_t incrementReferenceCount() { return ++_refCount; }
  uint32_t decrementReferenceCount() { return --_refCount; }
  RefCounted() : native_cpu::object_base(), _refCount{1} {}
