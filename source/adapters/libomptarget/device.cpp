...
UR_APIEXPORT ur_result_t UR_APICALL urDeviceGet(ur_platform_handle_t hPlatform,
  ...
  const size_t NumDevices = ReturnDevices ? hPlatform->Devices.size() : 0;
...
UR_APIEXPORT ur_result_t UR_APICALL urDeviceGetInfo(ur_device_handle_t hDevice,
  ...
  auto getAttribute = [](CUdevice NativeDevice, CUdevice_attribute Attribute) {
    int value;
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&value, Attribute, NativeDevice) == CUDA_SUCCESS);
    return value;
  };

  CUdevice NativeDevice;
  OMPT_RETURN_ON_FAILURE(cuDeviceGet(&NativeDevice, hDevice->getID()));
  CUcontext Context;
  OMPT_RETURN_ON_FAILURE(cuDevicePrimaryCtxRetain(&Context, hDevice->getID()));
  ...
  case UR_DEVICE_INFO_MAX_COMPUTE_UNITS: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&ComputeUnits,
                             ...
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(ComputeUnits >= 0);
  ...
  case UR_DEVICE_INFO_MAX_WORK_GROUP_SIZE: {
    ...
    omptarget_adapter::assertion(MaxWorkGroupSize >= 0);
  ...
  case UR_DEVICE_INFO_MAX_CLOCK_FREQUENCY: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&ClockFreq, CU_DEVICE_ATTRIBUTE_CLOCK_RATE,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(ClockFreq >= 0);
  ...
  case UR_DEVICE_INFO_MAX_MEM_ALLOC_SIZE: {
    ...
    size_t Global = 0;
    omptarget_adapter::assertion(cuDeviceTotalMem(&Global, NativeDevice) ==
                                 CUDA_SUCCESS);
  ...
  case UR_DEVICE_INFO_IMAGE2D_MAX_HEIGHT: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&TexHeight,
                             ...
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(TexHeight >= 0);
    int SurfHeight = 0;
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&SurfHeight,
                             CU_DEVICE_ATTRIBUTE_MAXIMUM_SURFACE2D_HEIGHT,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(SurfHeight >= 0);
  ...
  case UR_DEVICE_INFO_IMAGE2D_MAX_WIDTH: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&TexWidth,
                             ...
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(TexWidth >= 0);
    int SurfWidth = 0;
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&SurfWidth,
                             CU_DEVICE_ATTRIBUTE_MAXIMUM_SURFACE2D_WIDTH,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(SurfWidth >= 0);
  ...
  case UR_DEVICE_INFO_IMAGE3D_MAX_DEPTH: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&TexDepth,
                             ...
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(TexDepth >= 0);
    int SurfDepth = 0;
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&SurfDepth,
                             CU_DEVICE_ATTRIBUTE_MAXIMUM_SURFACE3D_DEPTH,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(SurfDepth >= 0);
  ...
  case UR_DEVICE_INFO_GLOBAL_MEM_CACHE_SIZE: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&CacheSize, CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(CacheSize >= 0);
  ...
  case UR_DEVICE_INFO_GLOBAL_MEM_SIZE: {
    ...
    omptarget_adapter::assertion(cuDeviceTotalMem(&Bytes, NativeDevice) ==
                                 CUDA_SUCCESS);
  ...
  case UR_DEVICE_INFO_MAX_CONSTANT_BUFFER_SIZE: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&ConstantMemory,
                             ...
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(ConstantMemory >= 0);
    ...
    if (hDevice->maxLocalMemSizeChosen()) {
      ...
      omptarget_adapter::assertion(
          cuDeviceGetAttribute(&LocalMemSize,
                               ...
                               NativeDevice) == CUDA_SUCCESS);
      omptarget_adapter::assertion(LocalMemSize >= 0);
  ...
  case UR_DEVICE_INFO_ERROR_CORRECTION_SUPPORT: {
    ...
    omptarget_adapter::assertion((ECCEnabled == 0) | (ECCEnabled == 1));
  ...
  case UR_DEVICE_INFO_HOST_UNIFIED_MEMORY: {
    ...
    omptarget_adapter::assertion((IsIntegrated == 0) | (IsIntegrated == 1));
  ...
  case UR_DEVICE_INFO_NAME: {
    ...
    omptarget_adapter::assertion(cuDeviceGetName(Name, MaxDeviceNameLength,
                                                 NativeDevice) == CUDA_SUCCESS);
  ...
  case UR_DEVICE_INFO_DRIVER_VERSION: {
    ...
    return ReturnValue(stream.str().c_str());
  }
  ...
  case UR_DEVICE_INFO_USM_HOST_SUPPORT: {
    ...
    uint32_t Value = {};
    if (getAttribute(NativeDevice, CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING)) {
      // the device shares a unified address space with the host
      if (getAttribute(NativeDevice,
  ...
  case UR_DEVICE_INFO_USM_SINGLE_SHARED_SUPPORT: {
    ...
    uint32_t Value = {};
    if (getAttribute(NativeDevice, CU_DEVICE_ATTRIBUTE_MANAGED_MEMORY)) {
  ...
  case UR_DEVICE_INFO_ASYNC_BARRIER: {
    int Value = getAttribute(NativeDevice,
                             CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR) >= 8;
    return ReturnValue(static_cast<bool>(Value));
  }
  case UR_DEVICE_INFO_BACKEND_RUNTIME_VERSION: {
    int Major = getAttribute(NativeDevice,
                             CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR);
    int Minor = getAttribute(NativeDevice,
                             CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR);
  ...
  case UR_DEVICE_INFO_GLOBAL_MEM_FREE: {
    ...
    omptarget_adapter::assertion(cuMemGetInfo(&FreeMemory, &TotalMemory) ==
                                     CUDA_SUCCESS,
  ...
  case UR_DEVICE_INFO_MEMORY_CLOCK_RATE: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetAttribute(&Value, CU_DEVICE_ATTRIBUTE_MEMORY_CLOCK_RATE,
                             NativeDevice) == CUDA_SUCCESS);
    omptarget_adapter::assertion(Value >= 0);
  ...
  case UR_DEVICE_INFO_UUID: {
    ...
    omptarget_adapter::assertion(cuDeviceGetUuid_v2(&UUID, NativeDevice) ==
                                 CUDA_SUCCESS);
#else
    omptarget_adapter::assertion(cuDeviceGetUuid(&UUID, NativeDevice) ==
                                 CUDA_SUCCESS);
#endif
  ...
  case UR_DEVICE_INFO_MAX_REGISTERS_PER_WORK_GROUP: {
    ...
    // on a multiprocessor.
    int MaxRegisters{-1};
    OMPT_RETURN_ON_FAILURE(cuDeviceGetAttribute(
        &MaxRegisters, CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK,
        NativeDevice));

    omptarget_adapter::assertion(MaxRegisters >= 0);
  ...
  case UR_DEVICE_INFO_PCI_ADDRESS: {
    ...
    omptarget_adapter::assertion(
        cuDeviceGetPCIBusId(AddressBuffer, AddressBufferSize, NativeDevice) ==
        CUDA_SUCCESS);
    // CUDA API (8.x - 12.1) guarantees 12 bytes + \0 are written
    omptarget_adapter::assertion(strnlen(AddressBuffer, AddressBufferSize) ==
                                 12);
...
/* This could be implemented by bypassing libomptarget and dealing with CUDA
 * handles directly. But there is probably no point in doing so */
UR_APIEXPORT ur_result_t UR_APICALL
...
urDeviceGetGlobalTimestamps([[maybe_unused]] ur_device_handle_t hDevice,
  ...
  if (pDeviceTimestamp) {
    /* libomptarget does not provide a way to query the device timestamp. A
     * solution similar to what the CUDA adapter is doing also doesn't work
     * since there is no way to query the elapsed time between 2 events in
     * libomptarget. */
    *pDeviceTimestamp = 0;
  }
