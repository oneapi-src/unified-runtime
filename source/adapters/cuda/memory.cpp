...
UR_APIEXPORT ur_result_t UR_APICALL urMemBufferCreate(
  ...
  try {
    ...
    if ((flags & UR_MEM_FLAG_USE_HOST_POINTER) && EnableUseHostPtr) {
      ...
      // call cuMemAllocHost(&HostPtr, size)
      umf_result_t umf_result = umfMemoryProviderAlloc(
          hContext->memoryProviderHost, size, 0, &HostPtr);
...
ur_result_t allocateMemObjOnDeviceIfNeeded(ur_mem_handle_t Mem,
  ...
  if (Mem->isBuffer()) {
    ...
    if (Buffer.MemAllocMode == BufferMem::AllocMode::AllocHostPtr) {
      ...
      // call cuMemAlloc(&DevPtr, Buffer.Size)
      umf_result_t umf_result = umfMemoryProviderAlloc(
          hDevice->memoryProviderDevice, Buffer.Size, 0, (void **)&DevPtr);
      UR_CHECK_ERROR(umf::umf2urResult(umf_result));
    }
  } else {
    CUarray ImageArray{};
