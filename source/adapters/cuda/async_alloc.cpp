...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueUSMDeviceAllocExp(
  ...
  if (pPool) {
    ...
    CUresult res = cuMemPoolGetAttribute(
        pPool->getCudaPool(), CU_MEMPOOL_ATTR_RESERVED_MEM_CURRENT, value);
    std::cout << "attr: " << *((uint64_t *)value) << std::endl;
    UR_CHECK_ERROR(
    ...
    CUresult res1 = cuMemPoolGetAttribute(
        pPool->getCudaPool(), CU_MEMPOOL_ATTR_RESERVED_MEM_CURRENT, value1);
    std::cout << "attr: " << *((uint64_t *)value1) << std::endl;
  } else {
