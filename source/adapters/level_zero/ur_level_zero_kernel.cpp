...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueKernelLaunch(
    ...
    if (LocalWorkSize) {
        ...
        if (SuggestGroupSize) {
            ...
            for (int I : {0, 1, 2}) {
                ...
                GroupSize[I] =
                    std::min(size_t(GroupSize[I]), GlobalWorkSize[I]);
            ...
            urPrint("urEnqueueKernelLaunch: using computed WG size = {%d, %d, "
                    "%d}\n",
                    WG[0], WG[1], WG[2]);
        }
    }
    ...
    if (Queue->Device->ImmCommandListUsed && IndirectAccessTrackingEnabled) {
        ...
        ZE2UR_CALL(zeCommandListAppendLaunchKernel,
                   (CommandList->first, Kernel->ZeKernel,
                    &ZeThreadGroupDimensions, ZeEvent,
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelGetInfo(
    ...
    default:
        urPrint(
            "Unsupported ParamName in urKernelGetInfo: ParamName=%d(0x%x)\n",
            ParamName, ParamName);
        return UR_RESULT_ERROR_INVALID_VALUE;
    }
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelGetGroupInfo(
    ...
    case UR_KERNEL_GROUP_INFO_PREFERRED_WORK_GROUP_SIZE_MULTIPLE: {
        return ReturnValue(
            size_t{Device->ZeDeviceProperties->physicalEUSimdWidth});
    }
    case UR_KERNEL_GROUP_INFO_PRIVATE_MEM_SIZE: {
        return ReturnValue(
            uint32_t{Kernel->ZeKernelProperties->privateMemSize});
    }
    default: {
        urPrint(
            "Unknown ParamName in urKernelGetGroupInfo: ParamName=%d(0x%x)\n",
            ParamName, ParamName);
        return UR_RESULT_ERROR_INVALID_VALUE;
    }
    }
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelSetArgSampler(
    ...
    ZE2UR_CALL(
        ...
        (Kernel->ZeKernel, ArgIndex, sizeof(void *), &ArgValue->ZeSampler));
...
UR_APIEXPORT ur_result_t UR_APICALL urKernelCreateWithNativeHandle(
    ...
    try {
        Kernel = new ur_kernel_handle_t_(
            ZeKernel, Properties->isNativeHandleOwned, Context);
