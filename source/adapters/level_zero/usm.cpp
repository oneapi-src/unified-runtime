...
ur_result_t urUSMGetMemAllocInfo(
  ...
  case UR_USM_ALLOC_INFO_TYPE:
    return ReturnValue(Tag->Type);
  case UR_USM_ALLOC_INFO_DEVICE:
    return ReturnValue(Tag->Device);
...
ur_usm_pool_handle_t_::ur_usm_pool_handle_t_(ur_context_handle_t Context,
  ...
  for (auto &Desc : Descriptors) {
    ...
    auto &Tag = UmfPoolTags.emplace_back();
    Tag.UrPool = this;
    Tag.Type = Desc.type;
    Tag.Device = Desc.hDevice;
    auto UmfRet = umfPoolSetTag(Pool.get(), (void *)&Tag, nullptr);
    if (UmfRet) {
      logger::error("urUSMPoolCreate: failed to set a tag for UMF pool {}",
                    Pool.get());
      throw UsmAllocationException(umf2urResult(UmfRet));
    }
