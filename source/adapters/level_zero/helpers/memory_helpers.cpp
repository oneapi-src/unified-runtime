...
ze_memory_allocation_properties_t getMemoryAttrs(ze_context_handle_t hContext,
  ...
  ZE2UR_CALL_THROWS(zeMemGetAllocProperties,
                    (hContext, ptr, &zeMemoryAllocationProperties, hDevice));
  return zeMemoryAllocationProperties;
}
...
getMemoryBaseAddressAndSize(ze_context_handle_t hContext, void *ptr) {
  void *Base = nullptr;
  size_t Size = 0;
  ZE2UR_CALL_THROWS(zeMemGetAddressRange, (hContext, ptr, &Base, &Size));
  return {Base, Size};
}
...
bool maybeImportUSM(ze_driver_handle_t hTranslatedDriver,
  ...
  if (ZeUSMImport.Enabled && ptr != nullptr &&
      getMemoryAttrs(hContext, ptr, nullptr).type == ZE_MEMORY_TYPE_UNKNOWN) {
