...
UR_APIEXPORT ur_result_t UR_APICALL urInit(
    ur_device_init_flags_t DeviceFlags ///< [in] device initialization flags.
...
UR_APIEXPORT ur_result_t UR_APICALL urPlatformGet(
    ...
    if (NumPlatforms) {
        if (*NumPlatforms == 0) {
            ...
            *NumPlatforms =
                std::min(PiPlatformsCache->size(), (size_t)NumEntries);
        }
    }
...
ur_platform_handle_t_::getDeviceFromNativeHandle(ze_device_handle_t ZeDevice) {
    ...
    auto it =
        std::find_if(PiDevicesCache.begin(), PiDevicesCache.end(),
...
ur_result_t ur_platform_handle_t_::populateDeviceCacheIfNeeded() {
    ...
    try {
        ...
        for (uint32_t I = 0; I < ZeDeviceCount; ++I) {
            std::unique_ptr<ur_device_handle_t_> Device(new ur_device_handle_t_(
                ZeDevices[I], (ur_platform_handle_t)this));
...
UR_APIEXPORT ur_result_t UR_APICALL urPlatformGetBackendOption(
    ...
    const char **PlatformOption ///< [out] returns the correct platform specific
