    ...
    std::vector<ur_event_handle_t> EventListToCleanup;
    {
        ...
        if (QueueLocked &&
            ...
            auto CompletedEventIt = std::find(
                CmdListEvents.begin(), CmdListEvents.end(), UrCompletedEvent);
            if (CompletedEventIt != CmdListEvents.end()) {
                ...
                CmdListEvents.erase(CmdListEvents.begin(),
                                    CompletedEventIt + 1);
            }
        } else {
        ...
        for (const auto &QueueMap :
            ...
            for (const auto &QueueGroup : QueueMap) {
                if (Queue->Device->ImmCommandListUsed) {
                    ...
                    for (const auto &ImmCmdList :
                         QueueGroup.second.ImmCmdLists) {
                        if (ImmCmdList == Queue->CommandListMap.end()) {
                            continue;
                        }

                        auto EventList = ImmCmdList->second.EventList;
                        for (auto It = EventList.crbegin();
                             It != EventList.crend(); It++) {
                            ze_result_t ZeResult = ZE_CALL_NOCHECK(
                                zeEventQueryStatus, ((*It)->ZeEvent));
                            if (ZeResult == ZE_RESULT_NOT_READY) {
                                return ReturnValue(false);
                            } else if (ZeResult != ZE_RESULT_SUCCESS) {
                                return ze2urResult(ZeResult);
...
UR_APIEXPORT ur_result_t UR_APICALL urQueueRelease(
    ...
    for (auto &Event : EventListToCleanup) {
        ...
        UR_CALL(
            urEventReleaseInternal(reinterpret_cast<ur_event_handle_t>(Event)));
    }
...
UR_APIEXPORT ur_result_t UR_APICALL urQueueFinish(
    ...
    ur_queue_handle_t_ *UrQueue =
        reinterpret_cast<ur_queue_handle_t_ *>(hQueue);
    ...
    if (UrQueue->Device->ImmCommandListUsed) {
        ...
        for (auto &QueueMap : {UrQueue->ComputeQueueGroupsByTID,
                               UrQueue->CopyQueueGroupsByTID}) {
        ...
        if (BatchSizeStrVal > 0) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (IsCopy) {
                        urPrint("SYCL_PI_LEVEL_ZERO_COPY_BATCH_SIZE: failed to "
                                "parse value\n");
                    } else {
                        urPrint("SYCL_PI_LEVEL_ZERO_BATCH_SIZE: failed to "
                                "parse value\n");
                    }
                    break;
                }
                ...
                if (IsCopy) {
                    urPrint("SYCL_PI_LEVEL_ZERO_COPY_BATCH_SIZE: dynamic batch "
                            "param "
                            "#%d: %d\n",
                            (int)Ord, (int)Val);
                } else {
                    urPrint("SYCL_PI_LEVEL_ZERO_BATCH_SIZE: dynamic batch "
                            "param #%d: %d\n",
                            (int)Ord, (int)Val);
                }
            };
            ...
            if (IsCopy) {
                urPrint("SYCL_PI_LEVEL_ZERO_COPY_BATCH_SIZE: ignored negative "
                        "value\n");
            } else {
                urPrint(
...
ur_queue_handle_t_::ur_queue_handle_t_(
    ...
    pi_queue_group_t ComputeQueueGroup{
        reinterpret_cast<ur_queue_handle_t>(this), queue_type::Compute};
    ...
    if (ComputeQueueGroupInfo.ZeIndex >= 0) {
        ...
        FilterUpperIndex =
            std::min((size_t)FilterUpperIndex,
    ...
    pi_queue_group_t CopyQueueGroup{reinterpret_cast<ur_queue_handle_t>(this),
                                    queue_type::MainCopy};
    const auto &Range =
        getRangeOfAllowedCopyEngines((ur_device_handle_t)Device);
...
ur_queue_handle_t_::executeCommandList(ur_command_list_ptr_t CommandList,
    ...
    if (!Device->ImmCommandListUsed) {
        ...
        // specified.
        auto &CommandBatch =
            UseCopyEngine ? CopyCommandBatch : ComputeCommandBatch;
        ...
        if (Device->ZeEventsScope == LastCommandInBatchHostVisible &&
            ...
            if (Result != CommandList->second.EventList.end()) {
                ...
                auto Res = createEventAndAssociateQueue(
                    ...
                    &HostVisibleEvent, UR_EXT_COMMAND_TYPE_USER, CommandList,
...
ur_queue_handle_t_::resetDiscardedEvent(ur_command_list_ptr_t CommandList) {
    if (LastCommandEvent && LastCommandEvent->IsDiscarded) {
        ZE2UR_CALL(zeCommandListAppendBarrier, (CommandList->first, nullptr, 1,
        ...
        if (LastCommandEvent->isHostVisible()) {
            PiEvent->HostVisibleEvent =
                reinterpret_cast<ur_event_handle_t>(PiEvent);
        }

        UR_CALL(
            addEventToQueueCache(reinterpret_cast<ur_event_handle_t>(PiEvent)));
    }
...
ur_result_t ur_queue_handle_t_::synchronize() {
    ...
    if (LastCommandEvent) {
        ...
        if (isInOrderQueue() && !LastCommandEvent->IsDiscarded) {
            ...
            for (auto &QueueMap :
                 {ComputeQueueGroupsByTID, CopyQueueGroupsByTID}) {
                for (auto &QueueGroup : QueueMap) {
                    if (Device->ImmCommandListUsed) {
...
ur_result_t createEventAndAssociateQueue(ur_queue_handle_t Queue,
    ...
    *Event = IsInternal ? Queue->getEventFromQueueCache(HostVisible.value())
                        : nullptr;
...
ur_result_t ur_queue_handle_t_::executeOpenCommandList(bool IsCopy) {
    ...
    if (hasOpenCommandList(IsCopy)) {
        ...
        auto Res =
            executeCommandList(CommandBatch.OpenCommandList, false, false);
        CommandBatch.OpenCommandList = CommandListMap.end();
        return Res;
    }
        ...
        if (isInOrderQueue()) {
            ...
            for (auto it = EventList.rbegin(); it != EventList.rend(); ++Iter) {
                if (!EventCompleted(*it)) {
                    if (Bisect > 1 &&
                        Iter < 3) { // Heuristically limit by 3 checks
                        Bisect >>= 1;
                        it += Bisect;
                        continue;
                    }
...
ur_queue_handle_t_::eventOpenCommandList(ur_event_handle_t Event) {
    ...
    if (hasOpenCommandList(IsCopy{false})) {
        ...
        if (std::find(ComputeEventList.begin(), ComputeEventList.end(),
                      Event) != ComputeEventList.end()) {
...
ur_queue_handle_t_::getQueueGroup(bool UseCopyEngine) {
    auto &Map =
        (UseCopyEngine ? CopyQueueGroupsByTID : ComputeQueueGroupsByTID);
    return Map.get();
}
...
uint32_t ur_queue_handle_t_::pi_queue_group_t::getQueueIndex(
    ...
    *QueueGroupOrdinal = Queue->Device->QueueGroup[QueueType].ZeOrdinal;
    ...
    if (QueueType == queue_type::LinkCopy &&
        Queue->Device->hasMainCopyEngine()) {
...
int32_t ur_queue_handle_t_::pi_queue_group_t::getCmdQueueOrdinal(
    ...
    if (QueueType != queue_type::Compute) {
        QueueType =
            (ZeQueues[0] == CmdQueue && Queue->Device->hasMainCopyEngine())
                ? queue_type::MainCopy
                : queue_type::LinkCopy;
    }
