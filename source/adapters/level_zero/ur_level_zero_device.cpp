...
UR_APIEXPORT ur_result_t UR_APICALL urDeviceGetInfo(
    ...
    case UR_DEVICE_INFO_MAX_MEM_ALLOC_SIZE:
        return ReturnValue(
            uint64_t{Device->ZeDeviceProperties->maxMemAllocSize});
    ...
    case UR_DEVICE_INFO_PARTITION_TYPE: {
        ...
        if (Device->isCCS()) {
            ...
            } PartitionProperties = {{UR_DEVICE_PARTITION_BY_CSLICE,
    ...
    case UR_DEVICE_INFO_GLOBAL_MEM_CACHE_SIZE:
        return ReturnValue(
            uint64_t{Device->ZeDeviceCacheProperties->cacheSize});
    ...
    case UR_DEVICE_INFO_MAX_SAMPLERS:
        return ReturnValue(
            uint32_t{Device->ZeDeviceImageProperties->maxSamplers});
    ...
    case UR_DEVICE_INFO_IMAGE2D_MAX_WIDTH:
        return ReturnValue(
            size_t{Device->ZeDeviceImageProperties->maxImageDims2D});
    case UR_DEVICE_INFO_IMAGE2D_MAX_HEIGHT:
        return ReturnValue(
            size_t{Device->ZeDeviceImageProperties->maxImageDims2D});
    case UR_DEVICE_INFO_IMAGE3D_MAX_WIDTH:
        return ReturnValue(
            size_t{Device->ZeDeviceImageProperties->maxImageDims3D});
    ...
    case UR_DEVICE_INFO_USM_SYSTEM_SHARED_SUPPORT: {
        ...
        case UR_DEVICE_INFO_USM_SINGLE_SHARED_SUPPORT:
            return ReturnValue(
                MapCaps(Props->sharedSingleDeviceAllocCapabilities));
        case UR_DEVICE_INFO_USM_CROSS_SHARED_SUPPORT:
            return ReturnValue(
                MapCaps(Props->sharedCrossDeviceAllocCapabilities));
        case UR_DEVICE_INFO_USM_SYSTEM_SHARED_SUPPORT:
    ...
    case UR_DEVICE_INFO_PCI_ADDRESS: {
        ...
        ZE2UR_CALL(zesDevicePciGetProperties,
                   (ZeDevice, &ZeDevicePciProperties));
    ...
    case UR_DEVICE_INFO_GLOBAL_MEM_FREE: {
        ...
        if (MemCount != 0) {
            ...
            for (auto &ZesMemHandle : ZesMemHandles) {
                ...
                ZE2UR_CALL(zesMemoryGetProperties,
                           (ZesMemHandle, &ZesMemProperties));
    ...
    case UR_DEVICE_INFO_MEMORY_CLOCK_RATE: {
        ...
        auto MinIt = std::min_element(
            Device->ZeDeviceMemoryProperties->first.begin(),
    ...
    case UR_EXT_DEVICE_INFO_GPU_EU_COUNT_PER_SUBSLICE:
        return ReturnValue(
            uint32_t{Device->ZeDeviceProperties->numEUsPerSubslice});
    case UR_EXT_DEVICE_INFO_GPU_HW_THREADS_PER_EU:
        return ReturnValue(
            uint32_t{Device->ZeDeviceProperties->numThreadsPerEU});
...
getRangeOfAllowedCopyEngines(const ur_device_handle_t &Device) {
    ...
    if (!EnvVar) {
        ...
        return std::pair<int, int>(0,
                                   INT_MAX); // All copy engines will be used.
    }
...
ur_result_t ur_device_handle_t_::initialize(int SubSubDeviceOrdinal,
    ...
    ZeDeviceProperties.Compute =
        [ZeDevice](ze_device_properties_t &Properties) {
    ...
    if (SubSubDeviceOrdinal >= 0) {
        ...
        if (QueueGroup[ur_device_handle_t_::queue_group_info_t::Compute]
                .ZeOrdinal < 0) {
            return UR_RESULT_ERROR_UNKNOWN;
        }
...
UR_APIEXPORT ur_result_t UR_APICALL urDevicePartition(
    ...
    //
    if (NumDevices != 0) {
        UR_ASSERT(NumDevices == EffectiveNumDevices,
