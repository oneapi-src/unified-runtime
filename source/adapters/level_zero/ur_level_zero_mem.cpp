...
ur_result_t enqueueMemCopyRectHelper(
    ...
    ZE2UR_CALL(zeCommandListAppendBarrier,
               (ZeCommandList, ZeEvent, 0, nullptr));
...
static ur_result_t enqueueMemFillHelper(ur_command_t CommandType,
    ...
    if (PreferCopyEngine && Device->hasCopyEngine()) {
        ...
        if (Device->hasLinkCopyEngine() &&
            Device->QueueGroup
                    [ur_device_handle_t_::queue_group_info_t::LinkCopy]
                        .ZeProperties.maxMemoryFillPatternSize < PatternSize) {
            PreferCopyEngine = false;
        }
    }
...
static ur_result_t getImageRegionHelper(_ur_image *Mem,
    ...
    UR_ASSERT(
        (ZeImageDesc.type == ZE_IMAGE_TYPE_1D && Origin->y == 0 &&
         Origin->z == 0) ||
...
static ur_result_t enqueueMemImageCommandHelper(
    ...
    if (CommandType == UR_COMMAND_MEM_IMAGE_READ) {
        ...
        UR_ASSERT(SlicePitch == 0 ||
                      SlicePitch == RowPitch * ZeSrcRegion.height,
                  UR_RESULT_ERROR_INVALID_IMAGE_SIZE);
        ...
        ZE2UR_CALL(zeCommandListAppendImageCopyToMemory,
                   (ZeCommandList, Dst, ur_cast<ze_image_handle_t>(ZeHandleSrc),
                    &ZeSrcRegion, ZeEvent, WaitList.Length,
                    WaitList.ZeEventList));
        ...
        UR_ASSERT(
            ...
            UR_RESULT_ERROR_INVALID_IMAGE_SIZE);
        UR_ASSERT(SlicePitch == 0 ||
                      SlicePitch == RowPitch * ZeDstRegion.height,
                  UR_RESULT_ERROR_INVALID_IMAGE_SIZE);
        ...
        ZE2UR_CALL(zeCommandListAppendImageCopyFromMemory,
                   (ZeCommandList, ur_cast<ze_image_handle_t>(ZeHandleDst), Src,
                    &ZeDstRegion, ZeEvent, WaitList.Length,
                    WaitList.ZeEventList));
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueMemBufferCopy(
    ...
    std::shared_lock<ur_shared_mutex> SrcLock(SrcBuffer->Mutex,
                                              std::defer_lock);
    std::scoped_lock<std::shared_lock<ur_shared_mutex>, ur_shared_mutex,
                     ur_shared_mutex>
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueMemBufferCopyRect(
    ...
    return enqueueMemCopyRectHelper(
        ...
        SrcOrigin, DstOrigin, SrcRegion, SrcRowPitch, DstRowPitch,
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueMemBufferMap(
    ...
    bool UseCopyEngine = false;
    {
        ...
        UR_CALL(createEventAndAssociateQueue(
            Queue, Event, UR_COMMAND_MEM_BUFFER_MAP,
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueMemUnmap(
    ...
    UR_CALL(Queue->Context->getAvailableCommandList(
        reinterpret_cast<ur_queue_handle_t>(Queue), CommandList,
        UseCopyEngine));
...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueUSMMemcpy2D(
    ...
    return enqueueMemCopyRectHelper( // TODO: do we need a new command type for
                                     // this?
        UR_COMMAND_MEM_BUFFER_COPY_RECT, Queue, Src, Dst, ZeroOffset,
        ZeroOffset, Region, SrcPitch, DstPitch, 0, /*SrcSlicePitch=*/
        0,                                         /*DstSlicePitch=*/
...
static ur_result_t ur2zeImageDesc(const ur_image_format_t *ImageFormat,
    ...
    default:
        urPrint(
            "urMemImageCreate: unsupported image data type: data type = %d\n",
    ...
    try {
        ...
        *Mem = reinterpret_cast<ur_mem_handle_t>(UrImage);
        ...
        if ((Flags & UR_MEM_FLAG_USE_HOST_POINTER) != 0 ||
            ...
            ZE2UR_CALL(zeCommandListAppendImageCopyFromMemory,
                       (Context->ZeCommandListInit, ZeImage, Host, nullptr,
                        nullptr, 0, nullptr));
        }
...
UR_APIEXPORT ur_result_t UR_APICALL urMemBufferCreate(
    ...
    if (ZeUSMImport.Enabled && Host != nullptr &&
        ...
        ze_device_handle_t ZeDeviceHandle;
        ZeStruct<ze_memory_allocation_properties_t>
        ...
        if ((Flags & UR_MEM_FLAG_USE_HOST_POINTER) != 0 ||
            ...
            if (Buffer->OnHost) {
                ...
                std::scoped_lock<ur_mutex> Lock(
                    Context->ImmediateCommandListMutex);
                ZE2UR_CALL(zeCommandListAppendMemoryCopy,
...
UR_APIEXPORT ur_result_t UR_APICALL urMemBufferPartition(
    ...
    if (Flags != UR_MEM_FLAG_READ_WRITE) {
        die("urMemBufferPartition: Level-Zero implements only read-write "
            "buffer,"
...
UR_APIEXPORT ur_result_t UR_APICALL urMemBufferCreateWithNativeHandle(
    ...
    if (ZeDevice) {
        ...
        UR_ASSERT(Context->isValidDevice(Device),
                  UR_RESULT_ERROR_INVALID_CONTEXT);
    }

    _ur_buffer *Buffer = nullptr;
    try {
        Buffer = new _ur_buffer(Context, Size, Device,
                                ur_cast<char *>(NativeMem), OwnNativeHandle);
    ...
    if (Device) {
        ...
        std::scoped_lock<ur_mutex> Lock(Context->ImmediateCommandListMutex);
        ZE2UR_CALL(zeCommandListAppendMemoryCopy,
                   (Context->ZeCommandListInit, ZeHandleDst, Ptr, Size, nullptr,
                    0, nullptr));
    }
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMHostAlloc(
    ...
    if (!UseUSMAllocator ||
        ...
        ur_result_t Res =
            USMHostAllocImpl(RetMem, Context, &Flags, Size, Align);
        if (IndirectAccessTrackingEnabled) {
            // Keep track of all memory allocations in the context
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMDeviceAlloc(
    ...
    if (!UseUSMAllocator ||
        ...
        ur_result_t Res = USMDeviceAllocImpl(RetMem, Context, Device, nullptr,
                                             Size, Alignment);
        if (IndirectAccessTrackingEnabled) {
            // Keep track of all memory allocations in the context
            Context->MemAllocs.emplace(std::piecewise_construct,
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMSharedAlloc(
    ...
    if (!UseUSMAllocator ||
        ...
        ((Alignment & (Alignment - 1)) != 0)) {
        ur_result_t Res =
            USMSharedAllocImpl(RetMem, Context, Device, &UsmHostFlags,
...
UR_APIEXPORT ur_result_t UR_APICALL urUSMGetMemAllocInfo(
    ...
    case UR_USM_ALLOC_INFO_BASE_PTR: {
        ...
        ZE2UR_CALL(zeMemGetAddressRange,
                   (Context->ZeContext, Ptr, &Base, nullptr));
        return ReturnValue(Base);
    }
    case UR_USM_ALLOC_INFO_SIZE: {
        size_t Size;
        ZE2UR_CALL(zeMemGetAddressRange,
                   (Context->ZeContext, Ptr, nullptr, &Size));
...
ur_result_t USMSharedMemoryAlloc::allocateImpl(void **ResultPtr, size_t Size,
                                               uint32_t Alignment) {
    return USMSharedAllocImpl(ResultPtr, Context, Device, nullptr, nullptr,
                              Size, Alignment);
}
...
ur_result_t USMSharedAllocImpl(void **ResultPtr, ur_context_handle_t Context,
    ...
    ZE2UR_CALL(zeMemAllocShared,
               (Context->ZeContext, &ZeDevDesc, &ZeHostDesc, Size, Alignment,
                Device->ZeDevice, ResultPtr));
...
ur_result_t USMFreeHelper(ur_context_handle_t Context, void *Ptr,
    ...
    if (!ZeDeviceHandle) {
        ...
        case ZE_MEMORY_TYPE_SHARED:
            ...
            SharedReadOnlyAllocsIterator =
                Context->SharedReadOnlyAllocs.find(Ptr);
            return DeallocationHelper(
                SharedReadOnlyAllocsIterator !=
...
ur_result_t _ur_buffer::getZeHandle(char *&ZeHandle, access_mode_t AccessMode,
    ...
    if (OnHost) {
        ...
        if (!HostAllocation.ZeHandle) {
            if (USMAllocatorConfigInstance.EnableBuffers) {
                ...
                UR_CALL(ZeHostMemAllocHelper(
                    reinterpret_cast<void **>(&ZeHandle), UrContext, Size));
            }
    ...
    if (!Allocation.ZeHandle) {
        if (!SingleRootDeviceBufferMigration && UrContext->SingleRootDevice &&
            ...
            UR_CALL(
                getZeHandle(ZeHandle, AccessMode, UrContext->SingleRootDevice));
            Allocation.ReleaseAction = allocation_t::keep;
            Allocation.ZeHandle = ZeHandle;
            ...
            if (USMAllocatorConfigInstance.EnableBuffers) {
                ...
                USMDesc.align = getAlignment();
                ur_usm_pool_handle_t Pool{};
                UR_CALL(urUSMDeviceAlloc(UrContext, Device, &USMDesc, Pool,
                                         Size,
                                         reinterpret_cast<void **>(&ZeHandle)));
            } else {
                Allocation.ReleaseAction = allocation_t::free_native;
                UR_CALL(
                    ZeDeviceMemAllocHelper(reinterpret_cast<void **>(&ZeHandle),
    ...
    if (!Allocation.Valid) {
        ...
        if (NeedCopy) {
            ...
            ZE2UR_CALL(zeDeviceCanAccessPeer,
                       (Device->ZeDevice,
                        LastDeviceWithValidAllocation->ZeDevice, &P2P));
            if (!P2P) {
                ...
                if (!HostAllocation.ZeHandle) {
                    ...
                    if (USMAllocatorConfigInstance.EnableBuffers) {
                        ...
                        HostAllocation.ReleaseAction =
                            allocation_t::free_native;
                        UR_CALL(ZeHostMemAllocHelper(&ZeHandleHost, UrContext,
                                                     Size));
                    }
                    HostAllocation.ZeHandle =
                        reinterpret_cast<char *>(ZeHandleHost);
                    HostAllocation.Valid = false;
                }
                std::scoped_lock<ur_mutex> Lock(
                    UrContext->ImmediateCommandListMutex);
                if (!HostAllocation.Valid) {
                    ZE2UR_CALL(zeCommandListAppendMemoryCopy,
                               (UrContext->ZeCommandListInit,
                                HostAllocation.ZeHandle, ZeHandleSrc, Size,
                                nullptr, 0, nullptr));
                ...
                std::scoped_lock<ur_mutex> Lock(
                    UrContext->ImmediateCommandListMutex);
                ZE2UR_CALL(zeCommandListAppendMemoryCopy,
                           (UrContext->ZeCommandListInit, ZeHandle, ZeHandleSrc,
...
ur_result_t _ur_buffer::free() {
    for (auto &Alloc : Allocations) {
        ...
        case allocation_t::free: {
            ...
            UR_CALL(USMFreeHelper(
                reinterpret_cast<ur_context_handle_t>(UrContext), ZeHandle));
            break;
        }
...
_ur_buffer::_ur_buffer(ur_context_handle_t Context, size_t Size, char *HostPtr,
    ...
    if (HostPtr) {
        ...
        if (ImportedHostPtr) {
            ...
            Allocations[nullptr].ReleaseAction =
                _ur_buffer::allocation_t::unimport;
        }
    }
