...
class Bucket {
    ...
    // Available list are entries in the pool.Each slab is available for a new
    ...
    // to USM or keep it allocated. A simple check for size of the Available
    // list is not sufficient to check whether any slab has been pooled yet.We
    // would have to traverse the entire Available listand check if any of them
    // is entirely free. Instead we keep a counter of entirely empty slabs
    // within the Available list to speed up the process of checking if a slab
    // in this bucket is already pooled.
...
auto Bucket::getAvailSlab(bool &FromPool) -> decltype(AvailableSlabs.begin()) {
    ...
    if (AvailableSlabs.size() == 0) {
        ...
        if ((*(AvailableSlabs.begin()))->getNumAllocated() == 0) {
            ...
            // Allocation from existing slab is treated as from pool for
            // statistics.
            FromPool = true;
        }
    }
...
void USMAllocContext::USMAllocImpl::deallocate(void *Ptr, bool &ToPool) {
    ...
    for (auto It = Slabs.first; It != Slabs.second; ++It) {
        // The slab object won't be deleted until it's removed from the map
        // which is protected by the lock, so it's safe to access it here.
        auto &Slab = It->second;
        if (Ptr >= Slab.getPtr() && Ptr < Slab.getEnd()) {
            // Unlock the map before freeing the chunk, it may be locked on
            // write there
            Lk.unlock();
        ...
        if (TitlePrinted) {
            try { // cannot throw in destructor
                std::cout << "Current Pool Size "
                          ...
                          << std::endl;
                const char *Label = MT;
                std::cout << "Suggested Setting: TODO_USM_ALLOCATOR=;"
                          << std::string(1, tolower(*Label))
                          << std::string(Label + 1) << ":" << HighBucketSize
