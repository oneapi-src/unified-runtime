...
class ur_context_info_v(IntEnum):
    ...
    DEVICES = 2                                     ## [::ur_device_handle_t...] The array of the device handles in the
                                                    ## context
    PLATFORM = 3                                    ## [::ur_platform_handle_t] The platform with which the context is
                                                    ## assocaited with
    REFERENCE_COUNT = 4                             ## [uint32_t] The reference count of the context
    ATOMIC_MEMORY_ORDER_CAPABILITIES = 5            ## [::ur_memory_order_flags_t] Supported memory order capabilities
    ATOMIC_MEMORY_SCOPE_CAPABILITIES = 6            ## [::ur_memory_scope_flags_t] Supported memory scopes
...
class ur_device_info_v(IntEnum):
    ...
    ADDRESS_BITS = 26                               ## uint32_t: address bits
    MAX_MEM_ALLOC_SIZE = 27                         ## uint64_t: max memory allocation size
    IMAGE_SUPPORTED = 28                            ## bool: images are supported
    MAX_READ_IMAGE_ARGS = 29                        ## uint32_t: max number of image objects arguments of a kernel declared
                                                    ## with the read_only qualifier
    MAX_WRITE_IMAGE_ARGS = 30                       ## uint32_t: max number of image objects arguments of a kernel declared
                                                    ## with the write_only qualifier
    MAX_READ_WRITE_IMAGE_ARGS = 31                  ## uint32_t: max number of image objects arguments of a kernel declared
                                                    ## with the read_write qualifier
    IMAGE2D_MAX_WIDTH = 32                          ## size_t: max width of Image2D object
    IMAGE2D_MAX_HEIGHT = 33                         ## size_t: max heigh of Image2D object
    IMAGE3D_MAX_WIDTH = 34                          ## size_t: max width of Image3D object
    IMAGE3D_MAX_HEIGHT = 35                         ## size_t: max height of Image3D object
    IMAGE3D_MAX_DEPTH = 36                          ## size_t: max depth of Image3D object
    IMAGE_MAX_BUFFER_SIZE = 37                      ## size_t: max image buffer size
    IMAGE_MAX_ARRAR_SIZE = 38                       ## size_t: max image array size
    MAX_SAMPLERS = 39                               ## uint32_t: max number of samplers that can be used in a kernel
    MAX_PARAMETER_SIZE = 40                         ## size_t: max size in bytes of all arguments passed to a kernel
    MEM_BASE_ADDR_ALIGN = 41                        ## uint32_t: memory base address alignment
    GLOBAL_MEM_CACHE_TYPE = 42                      ## ::ur_device_mem_cache_type_t: global memory cache type
    GLOBAL_MEM_CACHELINE_SIZE = 43                  ## uint32_t: global memory cache line size in bytes
    GLOBAL_MEM_CACHE_SIZE = 44                      ## uint64_t: size of global memory cache in bytes
    GLOBAL_MEM_SIZE = 45                            ## uint64_t: size of global memory in bytes
    GLOBAL_MEM_FREE = 46                            ## uint64_t: size of global memory which is free in bytes
    MAX_CONSTANT_BUFFER_SIZE = 47                   ## uint64_t: max constant buffer size in bytes
    MAX_CONSTANT_ARGS = 48                          ## uint32_t: max number of __const declared arguments in a kernel
    LOCAL_MEM_TYPE = 49                             ## ::ur_device_local_mem_type_t: local memory type
    LOCAL_MEM_SIZE = 50                             ## uint64_t: local memory size in bytes
    ERROR_CORRECTION_SUPPORT = 51                   ## bool: support error correction to gloal and local memory
    HOST_UNIFIED_MEMORY = 52                        ## bool: unifed host device memory
    PROFILING_TIMER_RESOLUTION = 53                 ## size_t: profiling timer resolution in nanoseconds
    ENDIAN_LITTLE = 54                              ## bool: little endian byte order
    AVAILABLE = 55                                  ## bool: device is available
    COMPILER_AVAILABLE = 56                         ## bool: device compiler is available
    LINKER_AVAILABLE = 57                           ## bool: device linker is available
    EXECUTION_CAPABILITIES = 58                     ## ::ur_device_exec_capability_flags_t: device kernel execution
                                                    ## capability bit-field
    QUEUE_ON_DEVICE_PROPERTIES = 59                 ## ::ur_queue_flags_t: device command queue property bit-field
    QUEUE_ON_HOST_PROPERTIES = 60                   ## ::ur_queue_flags_t: host queue property bit-field
    BUILT_IN_KERNELS = 61                           ## char[]: a semi-colon separated list of built-in kernels
    PLATFORM = 62                                   ## ::ur_platform_handle_t: the platform associated with the device
    REFERENCE_COUNT = 63                            ## uint32_t: reference count
    IL_VERSION = 64                                 ## char[]: IL version
    NAME = 65                                       ## char[]: Device name
    VENDOR = 66                                     ## char[]: Device vendor
    DRIVER_VERSION = 67                             ## char[]: Driver version
    PROFILE = 68                                    ## char[]: Device profile
    VERSION = 69                                    ## char[]: Device version
    BACKEND_RUNTIME_VERSION = 70                    ## char[]: Version of backend runtime
    EXTENSIONS = 71                                 ## char[]: Return a space separated list of extension names
    PRINTF_BUFFER_SIZE = 72                         ## size_t: Maximum size in bytes of internal printf buffer
    PREFERRED_INTEROP_USER_SYNC = 73                ## bool: prefer user synchronization when sharing object with other API
    PARENT_DEVICE = 74                              ## ::ur_device_handle_t: return parent device handle
    PARTITION_PROPERTIES = 75                       ## uint32_t: return a bit-field of partition properties
                                                    ## ::ur_device_partition_property_flags_t
    PARTITION_MAX_SUB_DEVICES = 76                  ## uint32_t: maximum number of sub-devices when the device is partitioned
    PARTITION_AFFINITY_DOMAIN = 77                  ## uint32_t: return a bit-field of affinity domain
                                                    ## ::ur_device_affinity_domain_flags_t
    PARTITION_TYPE = 78                             ## uint32_t: return a bit-field of ::ur_device_partition_property_flags_t
                                                    ## for properties specified in ::urDevicePartition
    MAX_NUM_SUB_GROUPS = 79                         ## uint32_t: max number of sub groups
    SUB_GROUP_INDEPENDENT_FORWARD_PROGRESS = 80     ## bool: support sub group independent forward progress
    SUB_GROUP_SIZES_INTEL = 81                      ## uint32_t[]: return an array of sub group sizes supported on Intel
                                                    ## device
    USM_HOST_SUPPORT = 82                           ## bool: support USM host memory access
    USM_DEVICE_SUPPORT = 83                         ## bool: support USM device memory access
    USM_SINGLE_SHARED_SUPPORT = 84                  ## bool: support USM single device shared memory access
    USM_CROSS_SHARED_SUPPORT = 85                   ## bool: support USM cross device shared memory access
    USM_SYSTEM_SHARED_SUPPORT = 86                  ## bool: support USM system wide shared memory access
    UUID = 87                                       ## char[]: return device UUID
    PCI_ADDRESS = 88                                ## char[]: return device PCI address
    GPU_EU_COUNT = 89                               ## uint32_t: return Intel GPU EU count
    GPU_EU_SIMD_WIDTH = 90                          ## uint32_t: return Intel GPU EU SIMD width
    GPU_EU_SLICES = 91                              ## uint32_t: return Intel GPU number of slices
    GPU_SUBSLICES_PER_SLICE = 92                    ## uint32_t: return Intel GPU number of subslices per slice
    MAX_MEMORY_BANDWIDTH = 93                       ## uint32_t: return max memory bandwidth in Mb/s
    IMAGE_SRGB = 94                                 ## bool: image is SRGB
    ATOMIC_64 = 95                                  ## bool: support 64 bit atomics
    ATOMIC_MEMORY_ORDER_CAPABILITIES = 96           ## uint32_t: atomics memory order capabilities
    BFLOAT16 = 97                                   ## bool: support for bfloat16
