...
typedef enum ur_queue_info_t {
    ...
    UR_QUEUE_INFO_DEVICE_DEFAULT = 2,  ///< [::ur_queue_handle_t] the current default queue of the underlying
                                       ///< device. Only returns a value if a queue associated with the
                                       ///< underlying device has been created with `::UR_QUEUE_FLAG_ON_DEVICE_DEFAULT`.
...
typedef enum ur_queue_flag_t {
    ...
    UR_QUEUE_FLAG_ON_DEVICE = UR_BIT(2),                     ///< Is a device queue. If this is enabled
                                                             ///< `::UR_QUEUE_FLAG_OUT_OF_ORDER_EXEC_MODE_ENABLE` must also be enabled.
