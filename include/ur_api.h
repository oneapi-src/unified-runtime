...
typedef enum ur_function_t {
    ...
    UR_FUNCTION_ENQUEUE_USM_DEVICE_ALLOC_EXP = 247,                       ///< Enumerator for ::urEnqueueUSMDeviceAllocExp
    UR_FUNCTION_ENQUEUE_USM_SHARED_ALLOC_EXP = 248,                       ///< Enumerator for ::urEnqueueUSMSharedAllocExp
    UR_FUNCTION_ENQUEUE_USM_HOST_ALLOC_EXP = 249,                         ///< Enumerator for ::urEnqueueUSMHostAllocExp
    UR_FUNCTION_ENQUEUE_USM_FREE_EXP = 250,                               ///< Enumerator for ::urEnqueueUSMFreeExp
    /// @cond
...
typedef enum ur_structure_type_t {
    ...
    UR_STRUCTURE_TYPE_EXP_ENQUEUE_USM_ALLOC_PROPERTIES = 0x3001,             ///< ::ur_exp_enqueue_usm_alloc_properties_t
...
typedef enum ur_device_info_t {
    ...
    UR_DEVICE_INFO_ENQUEUE_USM_ALLOCATIONS_EXP = 0x2022,             ///< [::ur_bool_t] returns true if the device supports USM allocation
                                                                     ///< enqueueing
    /// @cond
...
typedef enum ur_command_t {
    ...
    UR_COMMAND_ENQUEUE_USM_DEVICE_ALLOC_EXP = 0x2005,  ///< Event created by ::urEnqueueUSMDeviceAllocExp
    UR_COMMAND_ENQUEUE_USM_SHARED_ALLOC_EXP = 0x2006,  ///< Event created by ::urEnqueueUSMSharedAllocExp
    UR_COMMAND_ENQUEUE_USM_HOST_ALLOC_EXP = 0x2007,    ///< Event created by ::urEnqueueUSMHostAllocExp
    UR_COMMAND_ENQUEUE_USM_FREE_EXP = 0x2008,          ///< Event created by ::urEnqueueUSMFreeExp
    /// @cond
...
#pragma endregion
#endif
// Intel 'oneAPI' Unified Runtime Experimental API for enqueuing asynchronous USM allocations
#if !defined(__GNUC__)
...
typedef enum ur_exp_enqueue_usm_alloc_flag_t {
    UR_EXP_ENQUEUE_USM_ALLOC_FLAG_TBD = UR_BIT(0), ///< reserved for future use.
    /// @cond
...
typedef struct ur_exp_enqueue_usm_alloc_properties_t {
    ...
    void *pNext;                            ///< [in,out][optional] pointer to extension-specific structure
    ur_exp_enqueue_usm_alloc_flags_t flags; ///< [in] enqueue USM allocation flags
...
///         + `NULL == hQueue`
///     - ::UR_RESULT_ERROR_INVALID_ENUMERATION
///         + `NULL != pProperties && ::UR_EXP_ENQUEUE_USM_ALLOC_FLAGS_MASK & pProperties->flags`
///     - ::UR_RESULT_ERROR_INVALID_NULL_POINTER
///         + `NULL == ppMem`
///     - ::UR_RESULT_ERROR_OUT_OF_RESOURCES
///     - ::UR_RESULT_ERROR_INVALID_EVENT_WAIT_LIST
...
urEnqueueUSMDeviceAllocExp(
    ur_queue_handle_t hQueue,                                 ///< [in] handle of the queue object
    ur_usm_pool_handle_t pPool,                               ///< [in][optional] handle of the USM memory pool
    const size_t size,                                        ///< [in] minimum size in bytes of the USM memory object to be allocated
    const ur_exp_enqueue_usm_alloc_properties_t *pProperties, ///< [in][optional] pointer to the enqueue asynchronous USM allocation
                                                              ///< properties
    uint32_t numEventsInWaitList,                             ///< [in] size of the event wait list
    const ur_event_handle_t *phEventWaitList,                 ///< [in][optional][range(0, numEventsInWaitList)] pointer to a list of
                                                              ///< events that must be complete before the kernel execution.
    ...
    ur_event_handle_t *phEvent                                ///< [out][optional] return an event object that identifies the
...
urEnqueueUSMFreeExp(
    ur_queue_handle_t hQueue,                 ///< [in] handle of the queue object
    ur_usm_pool_handle_t pPool,               ///< [in][optional] handle of the USM memory pooliptor
    void *pMem,                               ///< [in] pointer to USM memory object
    uint32_t numEventsInWaitList,             ///< [in] size of the event wait list
    const ur_event_handle_t *phEventWaitList, ///< [in][optional][range(0, numEventsInWaitList)] pointer to a list of
                                              ///< events that must be complete before the kernel execution.
