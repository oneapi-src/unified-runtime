...
typedef enum _ur_memory_order_flag_t
{
...
typedef enum _ur_memory_scope_flag_t
{
...
typedef enum _ur_context_info_t
{
    ...
    UR_CONTEXT_INFO_DEVICES = 2,                    ///< [::ur_device_handle_t...] The array of the device handles in the
                                                    ///< context
    UR_CONTEXT_INFO_PLATFORM = 3,                   ///< [::ur_platform_handle_t] The platform with which the context is
                                                    ///< assocaited with
    UR_CONTEXT_INFO_REFERENCE_COUNT = 4,            ///< [uint32_t] The reference count of the context
    UR_CONTEXT_INFO_ATOMIC_MEMORY_ORDER_CAPABILITIES = 5,   ///< [::ur_memory_order_flags_t] Supported memory order capabilities
    UR_CONTEXT_INFO_ATOMIC_MEMORY_SCOPE_CAPABILITIES = 6,   ///< [::ur_memory_scope_flags_t] Supported memory scopes
...
typedef enum _ur_device_info_t
{
    ...
    UR_DEVICE_INFO_ADDRESS_BITS = 26,               ///< uint32_t: address bits
    UR_DEVICE_INFO_MAX_MEM_ALLOC_SIZE = 27,         ///< uint64_t: max memory allocation size
    UR_DEVICE_INFO_IMAGE_SUPPORTED = 28,            ///< bool: images are supported
    UR_DEVICE_INFO_MAX_READ_IMAGE_ARGS = 29,        ///< uint32_t: max number of image objects arguments of a kernel declared
                                                    ///< with the read_only qualifier
    UR_DEVICE_INFO_MAX_WRITE_IMAGE_ARGS = 30,       ///< uint32_t: max number of image objects arguments of a kernel declared
                                                    ///< with the write_only qualifier
    UR_DEVICE_INFO_MAX_READ_WRITE_IMAGE_ARGS = 31,  ///< uint32_t: max number of image objects arguments of a kernel declared
                                                    ///< with the read_write qualifier
    UR_DEVICE_INFO_IMAGE2D_MAX_WIDTH = 32,          ///< size_t: max width of Image2D object
    UR_DEVICE_INFO_IMAGE2D_MAX_HEIGHT = 33,         ///< size_t: max heigh of Image2D object
    UR_DEVICE_INFO_IMAGE3D_MAX_WIDTH = 34,          ///< size_t: max width of Image3D object
    UR_DEVICE_INFO_IMAGE3D_MAX_HEIGHT = 35,         ///< size_t: max height of Image3D object
    UR_DEVICE_INFO_IMAGE3D_MAX_DEPTH = 36,          ///< size_t: max depth of Image3D object
    UR_DEVICE_INFO_IMAGE_MAX_BUFFER_SIZE = 37,      ///< size_t: max image buffer size
    UR_DEVICE_INFO_IMAGE_MAX_ARRAR_SIZE = 38,       ///< size_t: max image array size
    UR_DEVICE_INFO_MAX_SAMPLERS = 39,               ///< uint32_t: max number of samplers that can be used in a kernel
    UR_DEVICE_INFO_MAX_PARAMETER_SIZE = 40,         ///< size_t: max size in bytes of all arguments passed to a kernel
    UR_DEVICE_INFO_MEM_BASE_ADDR_ALIGN = 41,        ///< uint32_t: memory base address alignment
    UR_DEVICE_INFO_GLOBAL_MEM_CACHE_TYPE = 42,      ///< ::ur_device_mem_cache_type_t: global memory cache type
    UR_DEVICE_INFO_GLOBAL_MEM_CACHELINE_SIZE = 43,  ///< uint32_t: global memory cache line size in bytes
    UR_DEVICE_INFO_GLOBAL_MEM_CACHE_SIZE = 44,      ///< uint64_t: size of global memory cache in bytes
    UR_DEVICE_INFO_GLOBAL_MEM_SIZE = 45,            ///< uint64_t: size of global memory in bytes
    UR_DEVICE_INFO_GLOBAL_MEM_FREE = 46,            ///< uint64_t: size of global memory which is free in bytes
    UR_DEVICE_INFO_MAX_CONSTANT_BUFFER_SIZE = 47,   ///< uint64_t: max constant buffer size in bytes
    UR_DEVICE_INFO_MAX_CONSTANT_ARGS = 48,          ///< uint32_t: max number of __const declared arguments in a kernel
    UR_DEVICE_INFO_LOCAL_MEM_TYPE = 49,             ///< ::ur_device_local_mem_type_t: local memory type
    UR_DEVICE_INFO_LOCAL_MEM_SIZE = 50,             ///< uint64_t: local memory size in bytes
    UR_DEVICE_INFO_ERROR_CORRECTION_SUPPORT = 51,   ///< bool: support error correction to gloal and local memory
    UR_DEVICE_INFO_HOST_UNIFIED_MEMORY = 52,        ///< bool: unifed host device memory
    UR_DEVICE_INFO_PROFILING_TIMER_RESOLUTION = 53, ///< size_t: profiling timer resolution in nanoseconds
    UR_DEVICE_INFO_ENDIAN_LITTLE = 54,              ///< bool: little endian byte order
    UR_DEVICE_INFO_AVAILABLE = 55,                  ///< bool: device is available
    UR_DEVICE_INFO_COMPILER_AVAILABLE = 56,         ///< bool: device compiler is available
    UR_DEVICE_INFO_LINKER_AVAILABLE = 57,           ///< bool: device linker is available
    UR_DEVICE_INFO_EXECUTION_CAPABILITIES = 58,     ///< ::ur_device_exec_capability_flags_t: device kernel execution
                                                    ///< capability bit-field
    UR_DEVICE_INFO_QUEUE_ON_DEVICE_PROPERTIES = 59, ///< ::ur_queue_flags_t: device command queue property bit-field
    UR_DEVICE_INFO_QUEUE_ON_HOST_PROPERTIES = 60,   ///< ::ur_queue_flags_t: host queue property bit-field
    UR_DEVICE_INFO_BUILT_IN_KERNELS = 61,           ///< char[]: a semi-colon separated list of built-in kernels
    UR_DEVICE_INFO_PLATFORM = 62,                   ///< ::ur_platform_handle_t: the platform associated with the device
    UR_DEVICE_INFO_REFERENCE_COUNT = 63,            ///< uint32_t: reference count
    UR_DEVICE_INFO_IL_VERSION = 64,                 ///< char[]: IL version
    UR_DEVICE_INFO_NAME = 65,                       ///< char[]: Device name
    UR_DEVICE_INFO_VENDOR = 66,                     ///< char[]: Device vendor
    UR_DEVICE_INFO_DRIVER_VERSION = 67,             ///< char[]: Driver version
    UR_DEVICE_INFO_PROFILE = 68,                    ///< char[]: Device profile
    UR_DEVICE_INFO_VERSION = 69,                    ///< char[]: Device version
    UR_DEVICE_INFO_BACKEND_RUNTIME_VERSION = 70,    ///< char[]: Version of backend runtime
    UR_DEVICE_INFO_EXTENSIONS = 71,                 ///< char[]: Return a space separated list of extension names
    UR_DEVICE_INFO_PRINTF_BUFFER_SIZE = 72,         ///< size_t: Maximum size in bytes of internal printf buffer
    UR_DEVICE_INFO_PREFERRED_INTEROP_USER_SYNC = 73,///< bool: prefer user synchronization when sharing object with other API
    UR_DEVICE_INFO_PARENT_DEVICE = 74,              ///< ::ur_device_handle_t: return parent device handle
    UR_DEVICE_INFO_PARTITION_PROPERTIES = 75,       ///< uint32_t: return a bit-field of partition properties
                                                    ///< ::ur_device_partition_property_flags_t
    UR_DEVICE_INFO_PARTITION_MAX_SUB_DEVICES = 76,  ///< uint32_t: maximum number of sub-devices when the device is partitioned
    UR_DEVICE_INFO_PARTITION_AFFINITY_DOMAIN = 77,  ///< uint32_t: return a bit-field of affinity domain
                                                    ///< ::ur_device_affinity_domain_flags_t
    UR_DEVICE_INFO_PARTITION_TYPE = 78,             ///< uint32_t: return a bit-field of ::ur_device_partition_property_flags_t
                                                    ///< for properties specified in ::urDevicePartition
    UR_DEVICE_INFO_MAX_NUM_SUB_GROUPS = 79,         ///< uint32_t: max number of sub groups
    UR_DEVICE_INFO_SUB_GROUP_INDEPENDENT_FORWARD_PROGRESS = 80, ///< bool: support sub group independent forward progress
    UR_DEVICE_INFO_SUB_GROUP_SIZES_INTEL = 81,      ///< uint32_t[]: return an array of sub group sizes supported on Intel
                                                    ///< device
    UR_DEVICE_INFO_USM_HOST_SUPPORT = 82,           ///< bool: support USM host memory access
    UR_DEVICE_INFO_USM_DEVICE_SUPPORT = 83,         ///< bool: support USM device memory access
    UR_DEVICE_INFO_USM_SINGLE_SHARED_SUPPORT = 84,  ///< bool: support USM single device shared memory access
    UR_DEVICE_INFO_USM_CROSS_SHARED_SUPPORT = 85,   ///< bool: support USM cross device shared memory access
    UR_DEVICE_INFO_USM_SYSTEM_SHARED_SUPPORT = 86,  ///< bool: support USM system wide shared memory access
    UR_DEVICE_INFO_UUID = 87,                       ///< char[]: return device UUID
    UR_DEVICE_INFO_PCI_ADDRESS = 88,                ///< char[]: return device PCI address
    UR_DEVICE_INFO_GPU_EU_COUNT = 89,               ///< uint32_t: return Intel GPU EU count
    UR_DEVICE_INFO_GPU_EU_SIMD_WIDTH = 90,          ///< uint32_t: return Intel GPU EU SIMD width
    UR_DEVICE_INFO_GPU_EU_SLICES = 91,              ///< uint32_t: return Intel GPU number of slices
    UR_DEVICE_INFO_GPU_SUBSLICES_PER_SLICE = 92,    ///< uint32_t: return Intel GPU number of subslices per slice
    UR_DEVICE_INFO_MAX_MEMORY_BANDWIDTH = 93,       ///< uint32_t: return max memory bandwidth in Mb/s
    UR_DEVICE_INFO_IMAGE_SRGB = 94,                 ///< bool: image is SRGB
    UR_DEVICE_INFO_ATOMIC_64 = 95,                  ///< bool: support 64 bit atomics
    UR_DEVICE_INFO_ATOMIC_MEMORY_ORDER_CAPABILITIES = 96,   ///< uint32_t: atomics memory order capabilities
    UR_DEVICE_INFO_BFLOAT16 = 97,                   ///< bool: support for bfloat16
