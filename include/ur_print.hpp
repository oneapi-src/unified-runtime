...
template <>
inline ur_result_t printFlag<ur_exp_enqueue_usm_alloc_flag_t>(std::ostream &os, uint32_t flag);
...
inline std::ostream &operator<<(std::ostream &os, enum ur_exp_enqueue_usm_alloc_flag_t value);
inline std::ostream &operator<<(std::ostream &os, [[maybe_unused]] const struct ur_exp_enqueue_usm_alloc_properties_t params);
...
inline ur_result_t printStruct(std::ostream &os, const void *ptr) {
    ...
    case UR_STRUCTURE_TYPE_EXP_ENQUEUE_USM_ALLOC_PROPERTIES: {
        const ur_exp_enqueue_usm_alloc_properties_t *pstruct = (const ur_exp_enqueue_usm_alloc_properties_t *)ptr;
        printPtr(os, pstruct);
    } break;
...
inline ur_result_t printFlag<ur_exp_enqueue_usm_alloc_flag_t>(std::ostream &os, uint32_t flag) {
    ...
    if ((val & UR_EXP_ENQUEUE_USM_ALLOC_FLAG_TBD) == (uint32_t)UR_EXP_ENQUEUE_USM_ALLOC_FLAG_TBD) {
...
/// @returns
///     std::ostream &
inline std::ostream &operator<<(std::ostream &os, const struct ur_exp_enqueue_usm_alloc_properties_t params) {
    ...
    ur::details::printFlag<ur_exp_enqueue_usm_alloc_flag_t>(os,
                                                            (params.flags));
